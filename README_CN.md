...
- 实验支线：sample 自身的嵌套 Objective-C 方法调用、跨线程 trace、一个 10 参数 Objective-C 调用，以及浮点参数/返回值现在也有自动化覆盖。
...
- `test_objc_msgsend_hook_experimental.sh` 会验证 sample 方法级 trace、跨线程事件、section 闭合情况、栈上传参的 Objective-C 调用，以及浮点参数和返回值。
